...
export const REPO = 'universalcompany/universal-company';
...
export function installerUrl(version: string, platform: string, arch: string): string {
  ...
  const file = platform === 'darwin' ? `Universal-Company-${v}-mac-${arch}.dmg`
    : platform === 'win32' ? `Universal-Company-${v}-win-x64-setup.exe`
    : `Universal-Company-${v}-linux-x86_64.AppImage`;
  ...
  switch (status?.state) {
    ...
    case 'downloaded':
      return {
        ...
        detail: `Restart Universal Company to finish updating from v${v}.`,
...
export function manualInstallSteps(platform: string): { os: string; steps: string[] } {
  if (platform === 'darwin') {
    return {
      os: 'macOS',
      steps: [
        'Open the .dmg and drag Universal Company onto Applications. Choose Replace when asked.',
  ...
  if (platform === 'win32') {
    return {
      ...
      steps: [
        ...
        'Open Universal Company again and pick the same project.'
      ]
    };
  }
  return {
